...
function Slot({ label, team, iso, color }: { label: string; team: string | null; iso: string | null; color: string }) {
  return (
    <div className="flex flex-col items-center gap-1 flex-1 min-w-0">
      ...
      <div className="h-8 flex items-center justify-center">
        {iso ? (
          iso.trim().toLowerCase().startsWith('http') ? (
            ...
            <img src={iso.trim()} alt={team || ''} className="w-7 h-7 rounded object-contain bg-slate-800" loading="lazy" />
          ) : (
      ...
      <span className="text-[10px] text-slate-300 truncate w-full text-center">{team ?? '—'}</span>
    </div>
  );
}
    ...
    <div className="space-y-8">
      <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-amber-500 font-semibold mb-1">Transparência — Palpites de Pódio</h3>
      ...
      {startedComps.length === 0 ? (
        <div className="text-slate-400 text-center py-12">Nenhuma competição começou ainda.</div>
      ) : (
        startedComps.map((comp) => (
          <div key={comp.key} className="space-y-3">
            <h4 className="text-white font-bold text-lg border-b border-slate-800 pb-2">{comp.name}</h4>
            {comp.predictions.length === 0 ? (
              <div className="text-slate-500 text-sm py-4">Nenhum palpite de pódio nesta competição.</div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {comp.predictions.map((p) => (
                  <Card key={`${comp.key}-${p.user_id}`} className="bg-slate-900 border-slate-800">
